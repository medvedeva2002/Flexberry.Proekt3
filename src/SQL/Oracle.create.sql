...
ALTER TABLE "ОценкаПодробно"
	ADD CONSTRAINT "ОценкаПодроб_9278" FOREIGN KEY ("Сотрудники") REFERENCES "Сотрудники" ("primaryKey");

CREATE INDEX "ОценкаПодроб_2145" on "ОценкаПодробно" ("Сотрудники");

ALTER TABLE "ОценкаПодробно"
	ADD CONSTRAINT "ОценкаПодроб_8151" FOREIGN KEY ("Оценка") REFERENCES "Оценка" ("primaryKey");

CREATE INDEX "ОценкаПодроб_6820" on "ОценкаПодробно" ("Оценка");

ALTER TABLE "ОценкаПодробно"
	ADD CONSTRAINT "ОценкаПодроб_8479" FOREIGN KEY ("ОтчётОбОценке") REFERENCES "ОтчётОбОценке" ("primaryKey");

CREATE INDEX "ОценкаПодроб_8503" on "ОценкаПодробно" ("ОтчётОбОценке");
